...
def update(frame):
    ...
    actual_data.append(df['faultNumber'].values[data_index])  
    ...
    ax1.scatter(range(len(actual_data)), list(actual_data), color='blue', label='Actual')
    ax2.scatter(range(len(predictions)), list(predictions), color='red', label='Predicted')
    
    
    ax1.set_title('Actual Fault Numbers')
